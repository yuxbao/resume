...
async function getRepoStars(url) {
  if (githubRepoCache[url]) return githubRepoCache[url].stargazers_count;
  try {
...
async function render(resume) {
  ...
  _.each(resume.publications, function (publication_info) {
    if (publication_info.releaseDate) {
      publication_info.releaseDate = moment(
        new Date(publication_info.releaseDate),
      ).format("MMM DD, YYYY");
    }
  });
  ...
  // Handle custom project_experience array if present, similar to projects
  if (resume.project_experience) {
    for (const project of resume.project_experience) {
      // Add custom logic here if needed, e.g. Github stars fetching if structure matches
      // But based on user input, structure is different.
    }
  }

  // Existing logic for standard projects array
  if (resume.projects) {
  ...
  // Merge resume properties into the root context for easier access in Handlebars
  var context = _.extend({}, resume, { css: css, resume: resume });
